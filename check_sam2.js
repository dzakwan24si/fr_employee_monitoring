async function check() {
  ...
  const res = await fetch(`${url}/employees?select=*&limit=1500`, { headers });
  const data = await res.json();
  const anja = data.filter(e => e['LOKASI TERAKHIR']?.trim() === 'ANJA' && (e.STATUS === 'Aktif' || e.STATUS === 'Eksis'));
  anja.forEach(e => {
    console.log(e.NAMA, "| Lokasi:", JSON.stringify(e['LOKASI TERAKHIR']));
  });
}
check();
